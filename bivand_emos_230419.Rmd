...
- On the one hand, information leaking between neighbouring spatial entities - spillover - needs to be taken into account

- On the other, we will see that looking carefully at spatial entities - observational units often not designed by the analyst, and understanding that such spillovers can occur, may lead us to clearer analysis
...
- Contents build on chapters 14-17 of @sdsr, also at https://r-spatial.org/book
...
- Boston housing value data set: a case of trying to study a problem when the support of the data probably does not match the problem
...
- Pre-dating spatial econometrics, two statisticians, @moran48 and @geary:54, had proposed measures that began to address the need to infer from maps
...
- @ord:10 reflects on their legacy, expressing doubt that the serious points raised by @granger:69 (and noted by @ripley:88) have been addressed adequately
...
- From the mid 1970's, two traditions developed, one handling the effects of spatial autocorrelation in modelling in ways analogous to time series, the other adding spatially structured random effects (in the statistical sense rather than panel econometrics sense) to models
...
- @lesage+pace:09 and their earlier and later work form the foundation for Markov chain Monte Carlo (MCMC) approaches; these by extension admit limited dependent variables in a convenient way
...
- These methods handle the spatially lagged response ${\mathbf W}{\mathbf y}$ by taking ${\mathbf W}{\mathbf X}$ and ${\mathbf W}{\mathbf W}{\mathbf X}$ (or more similar terms) as instruments
...
- Also note that acceptance on the Comprehensive R Archive Network (CRAN) only certifies that the package meets general standards for packages (licence declared, code runs examples and tests, functions are as least minimally documented), it does not confirm that packages do what they claim to do
...
- The SAS blogs follow general practice, not only in spatial econometrics, of comparative tabulation of model output and significance tests of fitted coefficients and goodness of fit statistics

- In the following, this practice will also be followed, but acknowledgement that multiple comparisons adjustments should have been applied is sensible

- Fitting multiple successive models to the same data, comparing the models for possible specifications, and choosing the estimated model suiting one's convictions is obviously highly subjective
...
```
The weakest weighted model output zone SDEM total impact of the air pollution covariate is still 2.5 times greater than the original calculation.
